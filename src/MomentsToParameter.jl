...
Mapping moments to parameters of a unviariate distribution.
...
For some distributions (8) relies on numerical root search if not analytically solvable.
Worst case: numerical root search and numerical computation of moments.
...
[`PTM`](@ref PTM) for the inverse mapping from parameters to moments. 
"""
...
function MTP(μ::Vector{T}, d::Chi)::Vector{Float64} where {T<:Real}
    ...
    return [find_zero(vars -> tf(vars, log(μ[1]) - log(2) / 2), (0.01, exp(d.ν)), Roots.A42(), atol=1e-05)]
end
