...
Hardcoded for commonly used distributions. If no closed form known, the function is based on
numerical computation of gradients and expectations.
...
function FInfo(d::NormalCanon{Float64})
    μ = d.η / d.λ
    σ = sqrt(1 / d.λ)
    H = [1/d.λ (-1)*d.η/d.λ^2; 0 (-1)/(2*d.λ^(3/2))]
    H' * FInfo(Normal(μ, σ)) * H
end # checked
